
default['aws'] = {}
default['aws']['load_balancer_name'] = "REPLACE ME"
default['aws']['key'] = "REPLACE ME"
default['aws']['secret'] = "REPLACE ME"
default['aws']['region'] = "us-west-2"

default['mustwin']['roles'] = ['WEB'] # WEB or WORKER

default['mustwin']['rails_prefix'] = 'webapp/'
default['mustwin']['app_name'] = 'REPLACE ME'

default['mustwin']['services']['WEB'] = ['puma']
default['mustwin']['services']['WORKER'] = []

default['mustwin']['rubies'] = ['ruby-2.0.0-p195']

# TODO: Drop these in data bags
default['mustwin']['ssh_key.pub'] = "REPLACE ME"
default['mustwin']['ssh_key'] = "REPLACE ME"
=begin
# TODO: Drop these in data bags
default[APP_NAME]['ssh_key.pub'] = "ssh-rsa  ubuntu@ip-172-31-38-242"
default[APP_NAME]['ssh_key'] = <<-EOF
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
EOF
=end
