...
app_root = "/mnt/www/#{node['mustwin']["app_name"]}" # TODO: may require late binding
...
template "/home/ubuntu/start_god.sh" do
  ...
  variables({
    ...
    :roles => node['mustwin']['roles'],
    :ruby => node['mustwin']['rubies'].first,
    :gemset => node['mustwin']['rubies'].first + '@' + node['mustwin']['app_name'],
    :logfile => "#{app_root}/shared/log/god.log",
    :pidfile => "#{app_root}/shared/pids/god.pid",
    :configfile => "#{app_root}/current/#{node['mustwin']['rails_prefix']}config/deploy/god_combined.rb",
...
deploy app_root do
  ...
  repo "git@github.com:MustWin/#{node['mustwin']['app_name']}.git"
  ...
  before_migrate do
    ...
    gemset = node['mustwin']['rubies'].first + '@' + node['mustwin']['app_name']
    node['mustwin']['rubies'].each do |ruby|
      # Install Ruby
      ...
      execute "Create gemset" do
        command(rvm_path + "use #{ruby}@#{node['mustwin']['app_name']} --create ")
      end
...
=begin
    execute "Trust rvmrc" do
      command "/usr/local/rvm/bin/rvm rvmrc trust #{current_release}/#{node['mustwin']['rails_prefix']}"
    end
=end
    # Bundle things
    ...
    execute "Bundle install" do
      command(rvm_path + " -c 'cd #{current_release}/#{node['mustwin']['rails_prefix']}; DEBUG_RESOLVER=true bundle install --verbose '")
    end

    # Run Migrations

    execute "Run migration" do
      command(rvm_path + " -c 'cd #{current_release}/#{node['mustwin']['rails_prefix']}; RAILS_ENV=#{node['env']['RAILS_ENV']} rake db:migrate --trace '")
    end

    # Remove existing log dir for symlink
    directory "#{current_release}/#{node['mustwin']['rails_prefix']}/log" do
      action :delete
      recursive true
    end
  end
  ...
  before_restart do
    ...
    execute "Create God configuration for all roles" do
      files = node['mustwin']['roles'].map{|role, b| "#{current_release}/#{node['mustwin']['rails_prefix']}/config/deploy/god_#{role.downcase}.rb" }
      cmd = 'cat ' + files.join(' ') + " > #{current_release}/#{node['mustwin']['rails_prefix']}config/deploy/god_combined.rb"
      command cmd
    end
    ...
    execute "Perform deploy related rake tasks" do
      command(rvm_path + " -c 'cd #{current_release}/#{node['mustwin']['rails_prefix']}; RAILS_ENV=#{node['env']['RAILS_ENV']} rake deploy:setup --trace '")
    end

    if node['mustwin']['roles'].include?('WEB')
      execute "Perform deploy related rake tasks" do
        command(rvm_path + " -c 'cd #{current_release}/#{node['mustwin']['rails_prefix']}; RAILS_ENV=#{node['env']['RAILS_ENV']} rake deploy:web --trace '")
      end
    ...
    elsif node['mustwin']['roles'].include?('WORKER')
      execute "Perform deploy related rake tasks" do
        command(rvm_path + " -c 'cd #{current_release}/#{node['mustwin']['rails_prefix']}; RAILS_ENV=#{node['env']['RAILS_ENV']} rake deploy:worker --trace '")
      end
    end

    node['mustwin']['roles'].each do |role, b|
      node['mustwin']['services'][role].each do |service|
  ...
  #migration_command "cd #{release_path}/#{node['mustwin']['rails_prefix']}; bundle exec rake db:migrate --trace"
  ...
  symlinks "tmp" => "#{node['mustwin']['rails_prefix']}tmp",
           "pids"=> "#{node['mustwin']['rails_prefix']}pids",
           "log" => "#{node['mustwin']['rails_prefix']}log"
end
