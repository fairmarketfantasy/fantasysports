class IndividualPredictionsController < ApplicationController
  def create
    ...
    prediction = current_user.individual_predictions.first_or_create(player_id: player.id,
                                                                     roster_id: params[:roster_id],
                                                                     market_id: params[:market_id],
                                                                     pt: IndividualPrediction::PT)
    params[:events].each do |event|
      event_prediction = prediction.event_predictions.first_or_create(event_type: event[:name],
                                                                      value: event[:value],
                                                                      diff: event[:diff])
