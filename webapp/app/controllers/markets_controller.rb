class MarketsController < ApplicationController
  ...
  def index
    ...
    @markets =  Market.where(['closed_at > ? AND closed_at <= ?  AND state IN(\'published\', \'opened\', \'closed\')', Time.now, week_market.closed_at]).page(page).order('closed_at asc').limit(10)
