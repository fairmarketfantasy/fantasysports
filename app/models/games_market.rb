class GamesMarket < ActiveRecord::Base
  ...
  def finish!
    ...
    when 'team_single_elimination' then
      ...
      StatEvent.create!(:player_stats_id => winning_mp.player_stats_id, :game_stats_id => game.stats_id, :activity => "Points Scored vs #{game.losing_team}", :point_value => 2 * game.winning_team_status['points'], :data => '{}')
      StatEvent.create!(:player_stats_id => winning_mp.player_stats_id, :game_stats_id => game.stats_id, :activity => "Points Against vs #{game.losing_team}", :point_value => -2 * game.losing_team_status['points'], :data => '{}')

      StatEvent.create!(:player_stats_id => losing_mp.player_stats_id, :game_stats_id => game.stats_id, :activity => "Points Scored vs #{game.winning_team}", :point_value => 2 * game.losing_team_status['points'], :data => '{}')
      StatEvent.create!(:player_stats_id => losing_mp.player_stats_id, :game_stats_id => game.stats_id, :activity => "Points Against vs #{game.winning_team}", :point_value => -2 * game.winning_team_status['points'], :data => '{}')
